...
def add_to_startup_windows(min_value, max_value, interface=None):
    ...
    with open(bat_path + '\\' + "open.bat", "w+") as bat_file:
        if interface == None:
            bat_file.write(rf'{pyton_path} "{file_path}\main.py" -s {min_value} -l {max_value}')
        else:
            bat_file.write(rf'{pyton_path} "{file_path}\main.py" -s {min_value} -l {max_value} -i {interface}')
